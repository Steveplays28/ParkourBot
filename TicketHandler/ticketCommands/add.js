  ...
  run: async (message, args, client, Ticket) => {
    ...
    if(!args[1] || !types.ticketTypes.includes(args[1])) {
      return message.channel.send(
        err_embed.addField(
          `Invalid ticket type`,
          args[1] ? `\`${args[1]}\` is not a valid ticket type` : `No ticket type specified`,
          false
        )
      )
    }
    ...
    const instance = new Ticket({
      ticketID: `${parseInt(message.channel.id).toString(16)}-${parseInt(message.id).toString(16)}`,
    ...
    message.channel.send(new Discord.MessageEmbed()
      ...
      .setDescription(`***ID:** ${parseInt(message.channel.id).toString(16)}-${parseInt(message.id).toString(16)}*`)
    )
  },
  name: "add"
}